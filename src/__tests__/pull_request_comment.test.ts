...
describe('Pull Request Comment Handler', () => {
  ...
  test('ignores unsupported event', async () => {
    (loadContext as jest.Mock).mockResolvedValue({ eventName: 'pull_request', payload: {}, repo: { owner: 'o', repo: 'r' } });
    await handlePullRequestComment();
    expect(getCommentThread).not.toHaveBeenCalled();
  });

  test('skips when comment thread not found', async () => {
    (getCommentThread as jest.Mock).mockResolvedValueOnce(null);
    await handlePullRequestComment();
    expect(runReviewCommentPrompt).not.toHaveBeenCalled();
  });

  test('skips when comment file not in changed files', async () => {
    (getCommentThread as jest.Mock).mockResolvedValueOnce({ file: 'other.ts', comments: [{ id: 1, line: 1, diff_hunk: 'h', user: { login: 'x' } }] });
    await handlePullRequestComment();
    expect(runReviewCommentPrompt).not.toHaveBeenCalled();
  });
